    ...
    try:
        ...
        if is_single_mode and len(df_price) > 200:
            try:
                ...
                train_data = ml_df.dropna() # 扣掉最後一天沒答案的
        ...
        if today['close'] > today['VWAP_20']: raw_score += 15 # 站上大戶成本加分
        ...
        if name_match.empty: st.error(f"❌ 找不到代號 {target_code}")
            ...
            if report:
                ...
                if enable_capital and my_capital:
                    ...
                    price_diff = report['收盤價'] - report['動態停利'] # 改用動態停利算風險
