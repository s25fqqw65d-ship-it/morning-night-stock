...
finmind_token = st.sidebar.text_input("輸入 FinMind Token", type="password")
...
def analyze_stock(stock_code, stock_name, dl, is_single_mode=False, price_filter="不限"):
    try:
        ...
        recent_10_high = df_price['max'].rolling(10).max().iloc[-1]
        weekly_target = recent_10_high if recent_10_high > today['close'] * 1.01 else today['close'] * 1.05
    ...
    if btn_single:
        ...
        if name_match.empty: st.error(f"❌ 找不到代號 {target_code}")
            ...
            if report:
                ...
                c2.metric("🎯 本週壓力目標", f"${report['本週目標']}")
